#!/usr/bin/env python3
"""
Compile LTX-2 DiT transformer backbone for Neuron (TP=4).

Produces compiled model files at OUTPUT_DIR that can be loaded by
NeuronLTX2BackboneApplication.load().

Usage:
  NEURON_RT_VISIBLE_CORES=0-3 python compile_dit.py [--output-dir DIR] \
      [--height H] [--width W] [--num-frames F] [--text-seq S]

Default output: /home/ubuntu/ltx2_nxdi_compiled_1024/
Compilation takes ~2-5 minutes on trn2.3xlarge.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Compile LTX-2 DiT backbone for Neuron")
    parser.add_argument(
        "--output-dir",
        default="/home/ubuntu/ltx2_nxdi_compiled_1024/",
        help="Directory to save compiled model (default: /home/ubuntu/ltx2_nxdi_compiled_1024/)",
    )
    ...
    print("\n[2/3] Downloading transformer weights...")
    ...
    print(f"  Path: {local_transformer_path}")
